...
- [x] For each `branch_summary` row when reloading linear history: **omit** the synthetic assistant “branch handoff” message if **`handoff_source_leaf_id` is non-empty and appears on that path**. That means the handoff still refers to an entry that remains on the active branch path (redundant with the transcript path / avoids naive double-injection in that situation).
- [x] If **`handoff_source_leaf_id` is empty** or **not on the path**, **inject** the branch-summary line as today (e.g. cross-session handoff where old-session entry IDs do not appear in the new file’s graph).
